...
export default function DietDetailPage() {
  ...
  const diet = MOCK_DIETS.find((d) => d.slug === slug);
  ...
  if (!diet) {
    return (
      <div className="text-center py-20 space-y-4">
        ...
        <Link href="/diets">
          <Button variant="primary">Back to Diets</Button>
        </Link>
      </div>
    );
  }
      ...
      <div className="p-8 rounded-3xl bg-gradient-to-r from-slate-900/90 via-slate-900/60 to-slate-900/90 border border-slate-800 space-y-4">
        ...
        <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-2 max-w-md">
          <div className="flex justify-between text-xs font-bold font-mono">
            <span className="text-purple-400">{diet.macroRatio.protein}% Protein</span>
            <span className="text-amber-400">{diet.macroRatio.carbs}% Carbs</span>
            <span className="text-rose-400">{diet.macroRatio.fat}% Fat</span>
          </div>
        ...
        <div className="pt-2">
          <Button
            ...
            onClick={() => setIsAdopted(!isAdopted)}
